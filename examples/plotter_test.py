...
backlight_pwm = pwmio.PWMOut(pimoroni_physical_feather_pins.pin21())
...
def test_fourlines(name, full_refresh=False):
    ...
    test_splotter.group.append(label.Label(terminalio.FONT,
                                           text="B: {:.0f}",
                                           color=green, x=50, y=5))
    test_splotter.group.append(label.Label(terminalio.FONT,
                                           text="C: {:.0f}",
                                           color=blue, x=110, y=5))
    test_splotter.group.append(label.Label(terminalio.FONT,
                                           text=name +
                                           (" CLS" if full_refresh else ""),
...
def test_twolinesfewdraws(name, full_refresh=False):
    """Only issue a draw every 4 updates to see how libs handles it.
